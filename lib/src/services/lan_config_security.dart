...
class User {
  ...
  /// Whether this username was auto-generated by FRITZ!OS 7.24+.
  ///
  /// When the box is updated to FRITZ!OS 7.24 and was previously configured
  /// for password-only login, it creates a user with a random name matching
  /// the pattern `fritz` followed by 4 digits (e.g. `fritz1337`).
  bool get isAutoGenerated => RegExp(r'^fritz\d{4}$').hasMatch(username);
...
class LanConfigSecurityService extends Tr64Service {
  ...
  /// Get the default username for login.
  ///
  /// Returns the username that the Fritz!Box web interface would
  /// pre-fill in its login form, following the logic described in
  /// AVM's "Empfehlungen zur Benutzerführung bei der Anmeldung":
  ///
  /// - Returns the last logged-in user's username, if available.
  /// - Returns `null` if the user list is empty (e.g. access from
  ///   outside the home network).
  Future<String?> getDefaultUsername() async {
    final users = await getUserList();
    if (users.isEmpty) return null;
    final lastUser = users.where((u) => u.lastUser).firstOrNull;
    return lastUser?.username ?? users.first.username;
  }
}
